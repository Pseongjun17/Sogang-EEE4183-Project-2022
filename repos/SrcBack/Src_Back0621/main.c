...
#ifdef RX_BOARD
static void boot_up_read_flash();
...
#define StartAddr ((u32) 0x0801FC00)
#define EndAddr ((u32) 0x0801FCFF)

void boot_up_read_flash(void)
{
    char config = 0;
    memcpy( &config, (char* ) StartAddr, sizeof( config ));
...
void store_data(void)
{
    ...
    unsigned int * p_w = (unsigned int *)&config;
    int byte_align  = 0;
    if( (sizeof(config) % 4) > 0)
        byte_align = 1; 

    FLASH_Unlock();
    FLASH_ErasePage(StartAddr);

    for( int i=0; i< sizeof( config ) / 4 + byte_align ; i++)
    {
